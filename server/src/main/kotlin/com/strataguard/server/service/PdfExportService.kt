...
import com.strataguard.server.controller.dto.GeneratePdfRequest
...
class PdfExportService(
    ...
    // Stateless generation from request payload — no S3, no DB lookup
    fun buildFromRequest(request: GeneratePdfRequest): ByteArray {
        val doc = PDDocument()
        val assessment = DisputeRiskService.Assessment(
            score = request.riskScore ?: 0.0,
            verdict = request.riskVerdict ?: "WEAK",
            factors = request.riskFactors ?: emptyMap(),
        )
        try {
            coverPageFromRequest(doc, request, assessment)
            assessmentPage(doc, assessment)
            if (request.incidents.isNotEmpty()) timelinePageFromRequest(doc, request.incidents)
            if (request.evidenceItems.isNotEmpty()) evidencePageFromRequest(doc, request.evidenceItems)
            legalPageForState(doc, request.state)
    ...
    // ── Stateless page builders (from request DTO) ────────────────────────

    private fun coverPageFromRequest(doc: PDDocument, req: GeneratePdfRequest, assessment: DisputeRiskService.Assessment) {
        val page = addPage(doc)
        PDPageContentStream(doc, page).use { cs ->
            navy(cs); cs.addRect(0f, 720f, 595f, 121f); cs.fill()
            white(cs)
            ...
            black(cs)
            var y = 690f
            y = labelValue(cs, "Dispute Type", disputeTypeLabel(req.disputeType), y)
            y = labelValue(cs, "Tribunal", "${req.tribunal} (${req.state})", y)
            y = labelValue(cs, "Status", req.status.replaceFirstChar { it.uppercase() }, y)
            req.strataAddress?.let { y = labelValue(cs, "Address", it, y) }
            req.filingDeadline?.let { labelValue(cs, "Filing Deadline", it, y) }
            when (assessment.verdict) {
                "STRONG" -> cs.setNonStrokingColor(0.18f, 0.63f, 0.34f)
                "MODERATE" -> cs.setNonStrokingColor(ORANGE_R, ORANGE_G, ORANGE_B)
                else -> cs.setNonStrokingColor(0.86f, 0.21f, 0.27f)
    ...
    private fun timelinePageFromRequest(doc: PDDocument, incidents: List<com.strataguard.server.controller.dto.IncidentPayload>) {
        val page = addPage(doc)
        PDPageContentStream(doc, page).use { cs ->
            sectionHeader(cs, "Incident Timeline", 3)
            var y = 680f
            for (incident in incidents.sortedBy { it.incidentDate }) {
                ...
                cs.addRect(50f, y - 2f, 4f, 16f); cs.fill()
                black(cs)
                drawText(cs, BOLD, 10f, incident.incidentDate, 62f, y + 10f)
                drawText(cs, BOLD, 10f, incident.title.take(80), 62f, y - 2f)
                incident.description?.take(100)?.let { drawText(cs, REGULAR, 8f, it, 62f, y - 14f) }
                val ev = incident.evidenceCount
                drawText(cs, REGULAR, 8f, "$ev evidence item${if (ev != 1) "s" else ""} attached", 62f, y - 26f)
                y -= 52f
            }
            footer(cs, 3)
    ...
    private fun evidencePageFromRequest(doc: PDDocument, evidence: List<com.strataguard.server.controller.dto.EvidencePayload>) {
        val page = addPage(doc)
        PDPageContentStream(doc, page).use { cs ->
            sectionHeader(cs, "Evidence Items", 4)
            var y = 680f
            for ((i, item) in evidence.withIndex()) {
                ...
                black(cs)
                drawText(cs, BOLD, 10f, "${i + 1}. ${(item.title ?: "Evidence").take(70)}", 50f, y)
                val capturedDate = java.time.Instant.ofEpochMilli(item.capturedAt)
                    .atZone(ZoneId.of("Australia/Sydney")).toLocalDate().format(DATE_FMT)
                drawText(cs, REGULAR, 8f, "Captured: $capturedDate  |  $verdictLabel", 62f, y - 14f)
                item.description?.take(110)?.let { drawText(cs, REGULAR, 8f, it, 62f, y - 26f) }
                if (verdict == "AI_GENERATED") {
                    cs.setNonStrokingColor(0.86f, 0.21f, 0.27f)
    ...
    private fun legalPageForState(doc: PDDocument, state: String) {
        val page = addPage(doc)
        PDPageContentStream(doc, page).use { cs ->
            sectionHeader(cs, "Legal Reference", 5)
            var y = 680f
            val (legislation, rights) = when (state.uppercase()) {
                "NSW" -> "Strata Schemes Management Act 2015 (NSW)" to listOf(
                    "Owners corporation must maintain common property in good repair (s106).",
                    "Tenants may seek orders at NCAT for breach of agreement or defect rectification.",
                    "Bond disputes: apply within 6 months of vacating or discovering the damage.",
